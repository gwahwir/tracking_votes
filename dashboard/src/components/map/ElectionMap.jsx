...
export const ElectionMap = ({ mapType = 'parlimen', useCartogram = false, onConstituencySelect }) => {
  ...
  const getCode = (feature) => {
    ...
    if (mapType === 'dun') return p.code_dun || p.constituency_code || p.code || p.id
    return p.code_parlimen || p.constituency_code || p.code || p.id
  }

  const getName = (feature) => {
    const p = feature.properties || {}
    if (mapType === 'dun') return p.dun || p.name || p.NAME || getCode(feature)
    return p.parlimen || p.name || p.NAME || getCode(feature)
  }
  ...
  return (
    <div className="election-map-container">
      ...
      {!geoJsonData ? (
        ...
        <MapContainer key={`${mapType}-${useCartogram}`} center={[1.485, 103.74]} zoom={8} className="election-map">
          <TileLayer
